    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border-t-4 border-blue-500">
                <div class="p-8">
                    <form action="{{ route('conteudos.update', $conteudo) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            <select name="tipo" id="tipo" required
                                ...
                                <option value="video" {{ old('tipo', $conteudo->tipo) === 'video' ? 'selected' : '' }}>Vídeo (YouTube, Vimeo)</option>
                                <option value="link" {{ old('tipo', $conteudo->tipo) === 'link' ? 'selected' : '' }}>Link Externo</option>
                                <option value="pdf" {{ old('tipo', $conteudo->tipo) === 'pdf' ? 'selected' : '' }}>Material PDF</option>
                                <option value="texto" {{ old('tipo', $conteudo->tipo) === 'texto' ? 'selected' : '' }}>Texto/Artigo</option>
                            </select>
                            @error('tipo')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-6" id="duracao-field">
                            ...
                            <input type="number" name="duracao_minutos" id="duracao_minutos" min="0"
                                   placeholder="Ex: 15"
                                   class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                   value="{{ old('duracao_minutos', $conteudo->duracao_segundos ? floor($conteudo->duracao_segundos / 60) : '') }}">
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="descricao" id="descricao" rows="4"
                                      ...
                                      class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('descricao', $conteudo->descricao) }}</textarea>
                            @error('descricao')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="observacoes" id="observacoes" rows="3"
                                      ...
                                      class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('observacoes', $conteudo->observacoes) }}</textarea>
                            @error('observacoes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                                ...
                                @foreach($alunos as $aluno)
                                    <label class="flex items-center py-2 hover:bg-gray-100 px-2 rounded cursor-pointer">
                                        <input type="checkbox" name="alunos_ids[]" value="{{ $aluno->id }}"
                                               ...
                                               {{ (is_array(old('alunos_ids')) && in_array($aluno->id, old('alunos_ids'))) || 
                                                  (is_array($conteudo->alunos_ids) && in_array($aluno->id, $conteudo->alunos_ids)) ? 'checked' : '' }}>
                        ...
                        <div class="mb-6">
                            ...
                            <select name="status" id="status" required
                                    class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                <option value="publicado" {{ old('status', $conteudo->status) === 'publicado' ? 'selected' : '' }}>Publicado (visível para alunos)</option>
                                <option value="rascunho" {{ old('status', $conteudo->status) === 'rascunho' ? 'selected' : '' }}>Rascunho (não visível)</option>
                                <option value="arquivado" {{ old('status', $conteudo->status) === 'arquivado' ? 'selected' : '' }}>Arquivado</option>
                            </select>
                        </div>
