<x-app-layout>
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600">Atualize os dados do aluno</p>
            </div>
            <x-button variant="outline" :href="route('alunos.show', $aluno)">
                Voltar
            </x-button>
        </div>
        ...
        <x-card>
            <form method="POST" action="{{ route('alunos.update', $aluno) }}">
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            ...
                            <x-input id="nome" name="nome" type="text" value="{{ old('nome', $aluno->nome) }}" required autofocus />
                            <x-input-error :messages="$errors->get('nome')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="email" value="E-mail" />
                            <x-input id="email" name="email" type="email" value="{{ old('email', $aluno->email) }}" />
                            <x-input-error :messages="$errors->get('email')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="telefone" value="Telefone" />
                            <x-input id="telefone" name="telefone" type="text" value="{{ old('telefone', $aluno->telefone) }}" placeholder="(00) 00000-0000" />
                            <x-input-error :messages="$errors->get('telefone')" class="mt-2" />
                        </div>

                        <div class="md:col-span-2">
                            <x-input-label for="endereco" value="Endereço" />
                            <x-input id="endereco" name="endereco" type="text" value="{{ old('endereco', $aluno->endereco) }}" />
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-input id="responsavel" name="responsavel" type="text" value="{{ old('responsavel', $aluno->responsavel) }}" />
                        ...
                        <div>
                            ...
                            <x-input id="telefone_responsavel" name="telefone_responsavel" type="text" value="{{ old('telefone_responsavel', $aluno->telefone_responsavel) }}" placeholder="(00) 00000-0000" />
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-input id="valor_aula" name="valor_aula" :money="true" value="{{ old('valor_aula', $aluno->valor_aula) }}" placeholder="0,00" />
                            <x-input-error :messages="$errors->get('valor_aula')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="data_inicio" value="Data de Início" />
                            <x-input id="data_inicio" name="data_inicio" type="date" value="{{ old('data_inicio', $aluno->data_inicio?->format('Y-m-d')) }}" />
                ...
                <div class="mb-6">
                    ...
                    <x-textarea id="observacoes" name="observacoes" rows="4" placeholder="Informações adicionais sobre o aluno...">{{ old('observacoes', $aluno->observacoes) }}</x-textarea>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        @php
                            $selectedTags = old('tags', $aluno->tags->pluck('id')->toArray());
                        @endphp
                        @foreach($tags as $tag)
                    ...
                    <div class="space-y-4">
                        <template x-for="(horario, index) in horarios" :key="index">
                            <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
                                <input type="hidden" x-bind:name="'horarios[' + index + '][id]'" x-model="horario.id">
                ...
                <div class="mb-6">
                    <label class="flex items-center">
                        <input type="checkbox" name="ativo" value="1" {{ old('ativo', $aluno->ativo) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 shadow-sm focus:ring-blue-500">
                ...
                <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                    <form method="POST" action="{{ route('alunos.destroy', $aluno) }}" onsubmit="return confirm('Tem certeza que deseja excluir este aluno? Esta ação não pode ser desfeita.');">
                        @csrf
                        @method('DELETE')
    ...
    <script>
        function horariosManager() {
            return {
                horarios: @json($aluno->horarios->where('ativo', true)->map(function($h) {
                    return [
                        'id' => $h->id,
