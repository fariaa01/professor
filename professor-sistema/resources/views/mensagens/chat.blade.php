<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8" style="max-width: 1400px; margin: 0 auto;">
        ...
        <div class="mb-6 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <a href="{{ route('alunos.index') }}" class="text-gray-500 hover:text-gray-700">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                    </svg>
                </a>
                <div>
                    <h1 class="text-3xl font-bold text-gray-900" id="alunoNome">Chat</h1>
                    <p class="text-sm text-gray-600" id="alunoEmail"></p>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        // Carregar informações do aluno e mensagens
        async function carregarChat() {
            try {
                const response = await fetch(API_BASE_URL + '/mensagens/' + alunoId, {
                    headers: {
                ...
                if (data.success) {
                    document.getElementById('alunoNome').textContent = data.data.aluno.nome;
                    document.getElementById('alunoEmail').textContent = data.data.aluno.email;
                    
                    console.log('Quantidade de mensagens:', data.data.mensagens.length);
        ...
        function renderizarMensagens(mensagens) {
            console.log('Renderizando mensagens:', mensagens);
            ...
            if (mensagens.length === 0) {
                ...
                mensagens.forEach(function(msg) {
                    ...
                    const align = isProfessor ? 'justify-end' : 'justify-start';
                    const bgStyle = isProfessor ? 'background: linear-gradient(to bottom right, #4f46e5, #4338ca); color: white; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);' : 'background: #f3f4f6; color: #111827; border: 1px solid #e5e7eb;';
                    ...
                    html += '<div class="flex ' + align + '" style="animation: fade-in 0.3s ease-out;">' +
                        ...
                        '</div>' +
                        '<div class="flex items-center gap-2 mt-1 ' + (isProfessor ? 'justify-end' : 'justify-start') + '">' +
                        '<p class="text-xs text-gray-500">' + msg.horario + '</p>' +
                        (isProfessor && msg.lida ? '<svg style="width: 0.75rem; height: 0.75rem; color: #3b82f6;" fill="currentColor" viewBox="0 0 20 20"><path d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"/></svg>' : '') +
                        '</div>' +
                        '</div>' +
                        '</div>';
                });
        ...
        async function marcarComoLidas() {
            try {
                await fetch(API_BASE_URL + '/mensagens/' + alunoId + '/marcar-lidas', {
                    method: 'PATCH',
                    headers: {
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
